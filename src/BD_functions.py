...
def reader(file):
    ...
    with open(file, encoding='utf-8')as f:
        ...
        for Position,Full_name,Gender,Date_Of_Birth,Country,Card_Type,Balance,Mortgage,Car_maker in reader:
            tuple = Info(int(Position),str(Full_name), str(Gender), datetime_parsing(Date_Of_Birth), str(Country),
...
def cars_of_a_country(register, Country):
    car = [s.Car_maker for s in register if s.Country==Country]
    return Counter(car)
